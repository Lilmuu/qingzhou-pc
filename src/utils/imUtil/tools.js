 ...
 * @return {Boolean}
 */
export function isObjectValueEqual (a, b, isOb) {
  const aProps = Object.getOwnPropertyNames(a)
  const bProps = Object.getOwnPropertyNames(b)

  if (isOb) {
    arrayRemove(aProps, '__ob__')
    arrayRemove(bProps, '__ob__')
  }
...
export const deepCopy = (obj) => {
  ...
  forEach(obj, (item, key) => {
    if (isArray(item)) {
      copy[key] = item.map(item => deepCopy(item))
    } else if (isObject(item)) {
      copy[key] = deepCopy(item)
    } else if (isFunction(item)) {
      copy[key] = item.bind(obj)
    } else {
      copy[key] = item
    }
  })
...
/**
 * @function objectDifference 计算两个对象数组的差值
 * @param arr1 对象数组
 * @param arr2 对象数组
 * @param key 比对key
 */
export function objectDifference (arr1, arr2, key) {
  let diff = [...arr1]
...
/**
 * @function formatSecond 格式化秒
 * @param second 秒
 */
export const formatSecond =  (second) => {
  const h = Math.floor(second / 3600) // 小时
  const m = Math.floor((second / 60) % 60) // 分钟
  const s = Math.floor(second % 60) // 秒
  let result = `${m}:${s}`
  if (h > 0) {
    result = `${h}:${result}`
  }
...
/**
 * @function arrayCheck 数组查重
 * @param arr 比对数组
 * @param val 比对内容
 * @param arrKey 比对的数组键值
 * @param valKey 比对的数组键值
 */
export const arrayCheck =  (arr = [], val = '', arrKey = '', valKey = arrKey) => {
  let isRepeat = false
