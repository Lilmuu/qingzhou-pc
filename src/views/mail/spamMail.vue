...
<template>
  <BasicContainer :useBorder="false">
    <div class="top-label">
      <MailTopHeader headerTitle="垃圾箱" @refresh="getList" @search="handleSearch"></MailTopHeader>
    </div>
...
<!--          <img src="@/assets/img/icon/mail.png" class="mail-type-icon" alt="">-->
          <i class="el-icon-info warningColor" style="margin-right: 5px;" v-show="scope.row.urgent === 1"></i>
          <span class="cursor" @click="changeRoute(scope.row)">{{ scope.row.from || '未填写' }}</span>
        </div>
      </template>
...
import { mailPageOption, inboxOption } from "@/const/dicData"
