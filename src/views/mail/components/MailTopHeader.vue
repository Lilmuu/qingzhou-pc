<template>
  <div class="mail-header">
    ...
    <div class="mailHeaderRowContainer">
      <div v-for="(item,idx) in list" :key="idx" class="mailHeaderRowContainer_list">
        <div :class="idx==index?'mailHeaderRowContainer_hover':'mailHeaderRowContainer_box'" @click="changeColor(idx,item.start)" v-if="idx!=3&&item.ht!=undefined">
          <img :src="item.img" alt="">
        </div>
        <DropDownMenu v-if="idx==3" :config="config" :entry="entry" @changeMenu="changeMenu"></DropDownMenu>
        <div v-if="idx!=list.length-1&&item.ht!=undefined" class="mailHeaderRowContainer_line"></div>
      </div>
...
<!--    </div>-->
    ...
    <el-dialog append-to-body
               ...
               custom-class='deleteEmailsCompletely'
               :close-on-click-modal="false">
      <!-- <div slot="title" class="dialog-header-row">
        <div class="dialog-tip"></div>
      ...
      <div class="flex-center" style="margin-top: 70px;">
        <div @click="deleteReDialog = false" class=" cancelButtonqx" style="margin-right: 10px;">取消</div>
        <div @click="handleDelMailRe" class=" cancelButtonqd">确定</div>
      </div>
    </el-dialog>
  </div>
</template>
...
import { ydd_dropDownOption } from "@/const/dicData"
...
export default {
  ...
  data() {
    return {
      ...
      index:6,
      list:[
        {img:require('@/assets/img/icon/mail/reply.png'),ht:this.config.hf,start:'hf'},
        {img:require('@/assets/img/icon/mail/replyAll.png'),ht:this.config.qbhf,start:'hfqb'},
        {img:require('@/assets/img/icon/mail/transmit.png'),ht:this.config.zf,start:'zf'},
        {img:require('@/assets/img/icon/mail/more.png')},
      ]
    }
  },
  mounted(){
  ...
  methods: {
    changeColor(index,start){
      console.log(start,123455645)
      this.index=index
      start=='hf'?this.handleHF():''
      start=='hfqb'?this.handleHFAll():''
      start=='zf'?this.handleZF():''
    },
    // 返回
    handleGoBack() {
    ...
    // 更爱菜单回调
    changeMenu(menu) {
      // console.log(this.$parent.$parent.$parent.$parent.activeName,1111111111111111111111)
      // this.$parent.$parent.$parent.$parent.activeName='writeMail'
      ...
      // this.$parent.$parent.$parent.$parent.activeName='writeMail'
        ...
        case 'zcbj':
          ...
          break;
        case 'bjw':
          this.bj_dropDownSelectChange(value,menu)
      // this.$parent.$parent.$parent.$parent.activeName='writeMail'
          break;
        case 'ydd':
    ...
    // 标记
    bj_dropDownSelectChange(tag,menu) {
    ...
    handleHFAll() {
      const isChecked = this.isSelected()
      if (!isChecked) return
      // console.log(this.mailIds,'this.mailIds')
      if (this.mailIds.length > 1) {
        this.$message.info('只能回复单个邮件')
        return
      }
